export function normalizeConversationResponse(responseBody) {
  ...
  return {
    ...
    cvAnalysisResult: data?.cv_analysis_result
      ...
      : null,
    careerAdviceResult: data?.career_advice_result
      ? normalizeCareerAdviceResult(data.career_advice_result)
      : null,
    coverLetterResult: data?.cover_letter_result
      ? normalizeCoverLetterResult(data.cover_letter_result)
      : null,
    jobSearchResult: data?.job_search_result
      ? normalizeJobSearchResult(data.job_search_result)
      : null,
...
function normalizeWorkflow(data) {
  ...
  return {
    workflowType: data.workflow_type ?? "single_agent",
...
function normalizeCoverLetterResult(data) {
  return {
    ...
    jobRequirementsAddressed: normalizeStringList(
      data?.job_requirements_addressed,
    ),
...
function normalizeCvAnalysisResult(data) {
  ...
  return {
    ...
    breakdown: {
      ...
      educationAndCredentials: toNumber(
        breakdown?.education_and_credentials,
      ),
    },
...
export function normalizeJobSearchResult(data) {
  if (!data || typeof data !== "object") {
    return {
      query: "",
      strategy: "postgres",
...
function normalizeJobSearchHit(hit) {
  return {
    ...
    matchedTerms: Array.isArray(hit?.matched_terms)
      ? hit.matched_terms
      : [],
