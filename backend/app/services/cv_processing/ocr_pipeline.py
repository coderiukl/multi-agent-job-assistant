...
class OCRPipeline:
    """Run OCR only for scanned pages or selected hybrid image regions."""
    ...
    def _ocr_image_regions(
        ...
        for index, block in enumerate(page_dict.get("blocks", [])):
            ...
            area_ratio = self._bbox_area(bbox) / page_area
            if area_ratio < self.settings.min_ocr_region_area_ratio:
                continue

            image = self.renderer.render_region(
                page,
                bbox=bbox,
